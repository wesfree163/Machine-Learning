...
# =============================
robots_only = r"C:\Users\Wes\Desktop\D2Exp\is_robot"   # cropped / close-up robots
rooms_dir   = r"C:\Users\Wes\Desktop\D2Exp\robots"      # room screenshots

# =============================
# CONFIGURATION
# =============================
patch_size = 134        # patch width & height (64–128 works well)
neg_per_image = 41     # how many background patches per room
confidence_threshold = 0.97  # SVM decision margin for detection
merge_distance = 50    # pixels to merge nearby detections
...
for f in os.listdir(robots_only):
    if f.lower().endswith(('.jpg', '.png')):
        im = cv2.imread(os.path.join(robots_only, f))
        if im is None:
            continue
...
for f in os.listdir(rooms_dir):
    if f.lower().endswith(('.jpg', '.png')):
        im = cv2.imread(os.path.join(rooms_dir, f))
        if im is None:
            continue
        h, w = im.shape[:2]
...
for fname in sorted(os.listdir(rooms_dir)):
    ...
    img_path = os.path.join(rooms_dir, fname)
